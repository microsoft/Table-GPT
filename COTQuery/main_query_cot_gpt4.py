...
def run_tasks(cache, cache_path, api_keys, api_bases, task_ids, test_data, model_name, max_tokens, n_jobs, batch_size):
    # process a batch of ids at every iteration
    batch_size = 1
    n_batches = (len(task_ids) - 1) // batch_size + 1
    gpt = GPT4(api_keys=api_keys, api_bases=api_bases, model_name=model_name, max_tokens=max_tokens, n_jobs=n_jobs)
...
parser.add_argument("--model", default="G4")
parser.add_argument("--temperature", type=float, default=0.0)
parser.add_argument("--batch_size", type=int, default=64)
parser.add_argument("--n_jobs", type=int, default=8)
args = parser.parse_args()

max_tokens_map = defaultdict(lambda: 400)

api_keys = utils.load_api_keys("api_keys_gpt4.txt")
api_bases = utils.load_api_keys("api_bases_gpt4.txt")

args.n_jobs = min(args.n_jobs, len(api_bases))
...
for task, test_data_group in test_data.groupby("task"):
    ...
    raise
    run_tasks(cache, cache_path, api_keys, api_bases, task_ids, test_data, args.model, max_tokens_map[task], args.n_jobs, args.batch_size)
