...
def get_random_fewshot_samples(n_samples, train_full, mv_column):
    ...
    for j in range(n_samples):
        df_sample = train_full.sample(n=6, random_state=j+2)
        train = df_sample.iloc[1:]
        test_row = df_sample.iloc[0:1]
        label = test_row[mv_column].values[0]
        mv_test_row = test_row.copy()
        mv_test_row[mv_column] = '[MISSING]'
        test_case = pd.concat([train, mv_test_row], axis=0).reset_index(drop=True)
...
def get_stanford_fewshot_samples(train_full, stanford_manual, mv_column):
    ...
    for j in range(len(stanford_manual)):
        df_sample = train_full.sample(n=5, random_state=j+2)
        train = df_sample.iloc[1:]
...
def build_buy_restaurant(data_dir, dataset, root_save_dir):
    ...
    train = train_full.sample(n=5, random_state=1)
    for i in range(len(test)):
        ...
        for j, (fewshot_test, fewshot_info) in enumerate(stanford_fewshot):
            fewshot_test.to_csv(makedir([save_dir, f"{dataset}Test", f"Table{i+1}", "stanford_fewshot_samples", f"sample_{j}"], "data.csv"), index=False)
            with open(os.path.join(save_dir, f"{dataset}Test", f"Table{i+1}", "stanford_fewshot_samples", f"sample_{j}", "info.json"), "w") as f:
                json.dump(fewshot_info, f, indent=4)