...
def inject_mv(df, random_state, mv_token="[MISSING]", mv_col_idx=None):
    ...
    if mv_col_idx is None:
        cand_columns = [(i, c) for i, c in enumerate(df.columns) if is_good_text_column(df[c].values)][-3:]
...
class DataImputationSynthesizer(BaseSynthesizer):
    def __init__(self):
        self.name = "DataImputation"
    ...
    def synthesize_one(self, df_full, random_state, save_dir):
        ...
        for j in range(10):
            ...
            df_mv, col_name, row_idx, col_idx, label = inject_mv(df_sample, random_state+j+1, mv_col_idx=col_idx)
            info = {
                "mv_column_name": col_name,
                "mv_row_idx": row_idx,
                "mv_col_idx": col_idx,
