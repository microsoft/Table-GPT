...
class HeaderValueMatching(BaseTableTask):
    ...
    def get_input(self, test_example):
        ...
        prompt = (
            "**Table Data:**\n"
            f"{self.serialize_df(df_no_header)}\n"
    ...
    def get_output_template(self, test_example):
        return self.answer_to_json(
            "column_headers",
            "<a list of headers for each column chosen from the candidate list>",
        )
