...
def read_blocked_pairs(
    ...
    labels = read_csv(split_path)
    
    tableA = tableA.add_suffix("_A")
    tableB = tableB.add_suffix("_B")

    mergedA = pd.merge(labels, tableA, right_on="id_A", left_on="ltable_id")
    merged = pd.merge(
        mergedA,
        tableB,
        right_on="id_B",
